...
const Card = ({ children, className = "" }) => (
  <div className={`rounded-2xl border border-[#E2DDD7] bg-white p-6 ${className}`}>{children}</div>
);
...
export default function PatientDashboard() {
  ...
  return (
    ...
    >
        ...
        <div className="space-y-6">
          <div className="grid lg:grid-cols-2 gap-6">
            <Card>
              ...
              {nextAppt ? (
                <>
                  <div className="font-display text-7xl text-[#1C3F39] font-mono tabular-nums leading-none">
                    #{String(nextAppt.queue_number).padStart(3, "0")}
                  </div>
                  <div className="text-sm text-[#5C6661] mt-3">with <span className="font-medium text-[#0A0F0D]">{nextAppt.doctor?.name}</span></div>
                  <Badge className={`mt-3 ${statusColors[nextAppt.status]}`}>{nextAppt.status.replaceAll("_", " ")}</Badge>
                </>
              ) : <div className="text-sm text-[#5C6661]">No active visit right now.</div>}
            </Card>
            <div className="rounded-2xl border border-[#E2DDD7] bg-[#1C3F39] text-[#F9F9F6] p-6 flex flex-col">
              ...
              <Button onClick={() => setOpenBook(true)} className="bg-[#B55B49] hover:bg-[#9b4a3b] text-[#F9F9F6] rounded-full self-start">
          ...
          <Card>
            ...
            <div className="space-y-2">
              ...
              {appts.map((a) => (
                <div key={a.id} className="flex flex-wrap items-center justify-between gap-3 p-3 rounded-xl border border-[#E2DDD7] hover:bg-[#F3EFE9] transition-colors">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-[#F3EFE9] flex items-center justify-center font-mono text-sm">#{a.queue_number}</div>
                    <div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {a.payment_status === "paid"
                      ...
                      : <Button size="sm" onClick={() => setOpenPay(a)} className="bg-[#B55B49] hover:bg-[#9b4a3b] text-[#F9F9F6] rounded-full h-8 px-3"><CreditCard size={14} className="mr-1.5" /> Pay</Button>}
                  </div>
                </div>
              ))}
            </div>
      ...
      {view === "records" && (
        <Card>
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => downloadPdf("/patient/history/pdf", "medical-history.pdf").catch(() => toast.error("Download failed"))}
                className="text-xs px-3 py-1.5 rounded-full border border-[#E2DDD7] hover:bg-[#F3EFE9] text-[#1C3F39]">Download PDF</button>
              <button onClick={() => { setProfile({ email: user.email?.includes("@patient.medilink") ? "" : user.email, phone: user.phone || "" }); setProfileOpen(true); }}
                className="text-xs px-3 py-1.5 rounded-full border border-[#E2DDD7] hover:bg-[#F3EFE9] text-[#1C3F39]">Edit profile</button>
          ...
          <div className="grid md:grid-cols-2 gap-3">
            {records.map((r) => (
              <div key={r.id} className="p-4 rounded-xl bg-[#F3EFE9] border border-[#E2DDD7]">
                ...
                {r.prescriptions?.length > 0 && (
                  <div className="mt-2 space-y-1">
                    {r.prescriptions.map((p, i) => (
                      <div key={i} className="text-xs font-mono flex items-center gap-2 text-[#1C3F39]"><Pill size={12} weight="duotone" /> {p.medicine} · {p.dosage} · {p.frequency}</div>
                    ))}
                  </div>
                )}
                ...
                <div className="mt-2 text-[10px] font-mono text-[#5C6661] flex items-center gap-1.5">
                  <span className={`w-1.5 h-1.5 rounded-full ${r.sync_status === "cloud" ? "bg-[#2D6A4F]" : "bg-[#D4A373]"} breathe`} />{r.sync_status}
                </div>
              </div>
            ))}
          </div>
      ...
      {view === "billing" && (
        <Card>
          <div className="flex items-center justify-between mb-4"><div className="overline">Receipts</div><CreditCard size={18} weight="duotone" color="#1C3F39" /></div>
          {receipts.length === 0 && <div className="text-sm text-[#5C6661]">No payments yet.</div>}
          <div className="grid md:grid-cols-2 gap-3">
            {receipts.map((r) => (
              <div key={r.txn_ref} className="flex items-center justify-between p-4 rounded-xl border border-[#E2DDD7]">
                <div>
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => downloadPdf(`/patient/receipts/${r.txn_ref}/pdf`, `receipt-${r.txn_ref}.pdf`).catch(() => toast.error("Download failed"))}
                    className="text-[11px] px-2 py-1 rounded-full border border-[#E2DDD7] hover:bg-[#F3EFE9] text-[#1C3F39]">PDF</button>
        ...
        <Card>
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => downloadPdf(`/patients/${user.id}/vaccinations/pdf`, "vaccination-certificate.pdf").catch(() => toast.error("Download failed"))}
              className="text-xs px-3 py-1.5 rounded-full border border-[#E2DDD7] hover:bg-[#F3EFE9] text-[#1C3F39]">Certificate PDF</button>
          </div>
          {vax.length === 0 && <div className="text-sm text-[#5C6661]">No vaccinations recorded.</div>}
          <div className="grid md:grid-cols-2 gap-3">
            {vax.map((v) => (
              <div key={v.id} className="flex items-center justify-between p-4 rounded-xl border border-[#E2DDD7]">
                <div>
      ...
      <Dialog open={openBook} onOpenChange={setOpenBook}>
        <DialogContent className="bg-[#F9F9F6] border-[#E2DDD7] max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            <div className="space-y-1.5">
              ...
              <Select value={booking.doctor_id} onValueChange={(v) => setBooking({ ...booking, doctor_id: v })}>
                <SelectTrigger className="border-[#E2DDD7]"><SelectValue placeholder="Choose a doctor" /></SelectTrigger>
            ...
            <div className="space-y-1.5"><Label>Reason for visit</Label><Textarea value={booking.reason} onChange={(e) => setBooking({ ...booking, reason: e.target.value })} placeholder="e.g. Persistent cough for 4 days" className="border-[#E2DDD7]" /></div>
          </div>
          <DialogFooter><Button disabled={!booking.doctor_id || !booking.scheduled_at || loading} onClick={book} className="bg-[#1C3F39] hover:bg-[#2D5A52] text-[#F9F9F6]">{loading ? "Booking…" : "Confirm booking"}</Button></DialogFooter>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent className="bg-[#F9F9F6] border-[#E2DDD7]">
          ...
          {openPay && !qrPay && (
            <div className="rounded-2xl border border-[#E2DDD7] bg-white p-5">
              ...
              <div className="text-sm text-[#5C6661] mt-2">{openPay.doctor?.name} · {new Date(openPay.scheduled_at).toLocaleString()}</div>
            </div>
          )}
          {qrPay && (
          ...
          <DialogFooter>
            {!qrPay
              ? <Button onClick={() => pay(openPay)} className="bg-[#1C3F39] hover:bg-[#2D5A52] text-[#F9F9F6]"><CreditCard size={16} className="mr-1.5" /> Show DuitNow QR</Button>
              : <Button onClick={confirmPay} disabled={confirming} className="bg-[#2D6A4F] hover:bg-[#255c43] text-[#F9F9F6]">{confirming ? "Checking…" : "I've paid"}</Button>}
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={profileOpen} onOpenChange={setProfileOpen}>
        <DialogContent className="bg-[#F9F9F6] border-[#E2DDD7]">
          ...
          <div className="space-y-3">
            <div className="space-y-1.5"><Label>Email (optional)</Label><Input value={profile.email} onChange={(e) => setProfile({ ...profile, email: e.target.value })} placeholder="you@email.com" className="border-[#E2DDD7]" /></div>
            <div className="space-y-1.5"><Label>Phone</Label><Input value={profile.phone} onChange={(e) => setProfile({ ...profile, phone: e.target.value })} placeholder="012-345 6789" className="border-[#E2DDD7]" /></div>
          </div>
          <DialogFooter><Button onClick={saveProfile} className="bg-[#1C3F39] hover:bg-[#2D5A52] text-[#F9F9F6]">Save</Button></DialogFooter>
        </DialogContent>
      </Dialog>
    </AppShell>
  );
